...
var dataProvider = (endpoint, options) => {
  ...
  if (options && options.axiosInterceptors) {
    if (options.axiosInterceptors.request)
      options.axiosInterceptors.request.forEach((value) => apiService.interceptors.request.use(value.onFulfilled, value.onRejected, value.options));
    if (options.axiosInterceptors.response)
      options.axiosInterceptors.response.forEach((value) => apiService.interceptors.response.use(value.onFulfilled, value.onRejected, value.options));
  }
  return {
...
// src/lib/isStringNumber.ts
var isStringNumber_default = (obj) => {
  return /^\d+$/.test(obj);
};
...
var getOneHandler = async (req, res, table, options) => {
  ...
  const row = await table.findUnique({
    where: {
      id: isStringNumber_default(req.body.params.id) ? +req.body.params.id : req.body.params.id
    },
    select: (_a = options == null ? void 0 : options.select) != null ? _a : void 0,
    include: (_b = options == null ? void 0 : options.include) != null ? _b : void 0
  });
  await ((_c = options == null ? void 0 : options.transform) == null ? void 0 : _c.call(options, row));
...
var updateHandler = async (req, res, table, options) => {
  const data = Object.entries(req.body.params.data).reduce((fields, [key, value]) => {
    var _a, _b;
    if (!isObject(value) && !((_a = options == null ? void 0 : options.skipFields) == null ? void 0 : _a.includes(key)) || ((_b = options == null ? void 0 : options.allowFields) == null ? void 0 : _b.includes(key)))
      fields[key] = value;
    return fields;
  }, {});
...
var defaultHandler = async (req, res, prisma, options) => {
  ...
  if (!prismaDelegate)
    throw new Error(`No model found for "${req.body.model || req.body.resource}"`);
  switch (req.body.method) {
    ...
    case "update": {
      return await updateHandler(req, res, prismaDelegate, options == null ? void 0 : options.update);
    }
    case "delete": {
